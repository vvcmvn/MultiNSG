...
class IndexNSG : public Index {
 public:
  ...
  const std::vector<std::vector<unsigned>>*  GetFinalGraph(){
    return &final_graph_;
  }
  virtual ~IndexNSG();
  ...
  struct GraphStats
  {
    ...
    double skewness;
    double kurtosis;
    double avg_first_to_last_ratio;
    double avg_first_to_mid_ratio;
  };
  ...
  QueryResult GetQueryResult(const float *query_data,
                             unsigned query_num,
                             unsigned K,
                             const std::vector<std::vector<unsigned>> &ground_truth,
                             unsigned L) const;

  void EvaluateGraph(const float *query_data,
                     unsigned query_num,
                     unsigned K,
